      ...
      <section className="mt-12 rounded-2xl border border-zinc-200/90 bg-[var(--card)] p-6 shadow-md dark:border-zinc-800">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            Each finished job stores <strong className="text-zinc-600 dark:text-zinc-300">accountId</strong>, the old handle
            (frozen when you started the job), and the new handle after TikTok + DB update. Expand a row for @before → @after.
          </p>
        </div>
        {historyLoading ? (
                ...
                >
                  <details className="group">
                    ...
                    <div className="border-t border-zinc-200 px-4 py-3 dark:border-zinc-700">
                      {job.items.length === 0 ? (
                        <p className="text-xs text-zinc-500">No per-account rows yet.</p>
                      ) : (
